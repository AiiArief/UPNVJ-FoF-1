...
public class VisionLook : MonoBehaviour
{
    ...
    EntityCharacterPlayer mainPlayer;
    ...
    {
        var playerPlayables = GameManager.Instance.playerManager.GetPlayerPlayableList();
        foreach (Entity player in playerPlayables)
        {
    ...
    private void _HandleVisionLookMesh()
    {
        ...
        for (int i = 0; i <= rayCount; i++)
        {
            ...
            {
                vertex = transform.InverseTransformPoint(transform.position + localOrigin + _GenerateVectorFromAngle(currentAngle));
            }
    ...
    private Vector3 _GenerateVectorFromAngle(float angle)
    {
        return Quaternion.AngleAxis(angle, Vector3.up) * new Vector3(0.0f, 0.0f, 1.0f);
    }
    ...
    private bool _CheckPlayerIsCloseEnough()
    {
        ...
        float distance = Vector3.Distance(mainPlayer.transform.position, m_bot.transform.position);
